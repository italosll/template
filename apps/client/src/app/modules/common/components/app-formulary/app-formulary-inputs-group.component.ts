...
import { SchemeContract } from "./scheme.contract";
        ...
        @for(input of inputs(); track input.name){
            ...
            @if(input.type === "image"){
                <mat-form-field 
                    ...
                    [formGroup]="form!"
                 >
                    <mat-label style="background: white; padding: 5px;">{{input.label}}</mat-label>
...
export class FormularyInputsGroupComponent{
    public inputs = input.required<SchemeContract[0]["inputs"]>();
    private _formGroupDirective = inject(FormGroupDirective, {skipSelf: true, optional: true})
