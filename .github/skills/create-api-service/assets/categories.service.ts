...
export class CategoriesService
  implements EntityService<Category, CreateCategoryDTO, UpdateCategoryDTO>
{
  constructor(
  ...
  async findAll(category?: Partial<FullCategoryDTO>): Promise<Category[]> {
    ...
    if (category?.id)
      queryBuilder.andWhere(`id LIKE :id`, { id: `%${category.id}%` });
    if (category?.name)
      queryBuilder.andWhere(`LOWER(name) LIKE LOWER(:name)`, {
        name: `%${category.name}%`,
      });
    if (category?.code)
      queryBuilder.andWhere(`code LIKE :code`, { code: `%${category.code}%` });

    if (category?.createdAt)
      queryBuilder.andWhere(`createdAt LIKE :createdAt`, {
        createdAt: `%${category.createdAt}%`,
      });
    if (category?.updatedAt)
      queryBuilder.andWhere(`updatedAt LIKE :updatedAt`, {
        updatedAt: `%${category.updatedAt}%`,
      });
    if (category?.deletedAt)
      queryBuilder.andWhere(`deletedAt LIKE :deletedAt`, {
        deletedAt: `%${category.deletedAt}%`,
      });
    if (category?.recoveredAt)
      queryBuilder.andWhere(`recoveredAt LIKE :recoveredAt`, {
        recoveredAt: `%${category.recoveredAt}%`,
      });
  ...
  async create(
    createEntity: CreateCategoryDTO
  ): Promise<CreateDefaultResponseDTO> {
    const registeredProduct = await this._categoryRepository.findOneBy({
  ...
  async update(
    updateEntity?: UpdateCategoryDTO
    ...
    const merged = this._categoryRepository.merge(
      registeredProduct,
      updateEntity ?? {}
    );
